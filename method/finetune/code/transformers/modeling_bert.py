...
class BertModel(BertPreTrainedModel):
    ...
    Return:
        ...
        # todo cuinan: group token
        batch, size, _ = embedding_output.shape
        new_embedding = embedding_output.clone()
        for b in range(batch):
            embs = embedding_output[b]
            group = token_group[b]
            p, q, i = 0, 0, 0
            while p < size:
                if group[p] == 0:
                    new_embedding[b][i] = embs[p]
                    p += 1
                    i += 1
                    continue
                q = p+1
...
def f_first(embs, p, q):
    # get the first embedding to represent the group
    return embs[p]
